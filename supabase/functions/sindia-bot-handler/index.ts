...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const length = useGlobal ? globalConfig?.sindia_response_length : condominioInfo?.sindia_response_length;
    const delay = useGlobal ? globalConfig?.sindia_delay_seconds : condominioInfo?.sindia_delay_seconds;
    ...
    const basePrompt = customPrompt || "Você é SINDIA. Responda APENAS baseado nos documentos fornecidos.";
    ...
    if (lowerQuestion.includes('regra') || lowerQuestion.includes('horário') || lowerQuestion.includes('piscina')) {
      if (hasDocs) aiResponse = "Com base no Regimento Interno, o horário de silêncio é das 22h às 08h.";
